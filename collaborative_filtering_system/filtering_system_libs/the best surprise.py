...
# MAGIC This notebook demonstrates building a collaborative filtering recommendation system using Python libraries Surprise and MLflow. The code is designed to train an SVD-based recommendation model, log metrics and models with MLflow, and provide a function for generating recommendations.

# COMMAND ----------

# MAGIC %md
# MAGIC ## 1.1 Installing Libraries
# MAGIC We start by installing the necessary libraries. **Surprise** is used for building the recommendation system, and **MLflow** is used for tracking and managing the machine learning lifecycle, including experimentation, reproducibility, and deployment.
...
# MAGIC %md
# MAGIC Here, we import the required libraries:
# MAGIC - **pandas** for data manipulation.
# MAGIC - **mlflow** and **mlflow.sklearn** for managing the ML lifecycle.
# MAGIC - **surprise** for building the recommendation system.
# MAGIC - **collections.defaultdict** for managing user-item interactions.
# MAGIC - **mlflow.models.signature** for model signature inference.
# MAGIC
...
# MAGIC We load the ratings and movies data into pandas DataFrames and ensure that **userId** and **movieId** are treated as strings.
...
# MAGIC %md
# MAGIC # 2. Training
# MAGIC In this section, we will define constants, transform data, split it into training and testing sets, and train the model.

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2.1 Const Definitions
# MAGIC We define the **Reader** object to specify the rating scale, which is between 0 and 5.
...
# MAGIC We load the data from the ratings DataFrame into the **surprise** format, which is required for training the model.
...
# MAGIC We split the data into training and testing sets. Here, 20% of the data is reserved for testing.
...
# MAGIC %md
# MAGIC ## 2.4 Training
# MAGIC We initialize the SVD algorithm and start an MLflow run to train the model and log the parameters and metrics. We calculate metrics such as RMSE, MAE, Precision@K, Recall@K, and F1@K and log them to MLflow.
...
# MAGIC We define a function to get recommendations for a specific user. This function identifies the movies that the user hasn't seen, predicts their ratings, and returns the top N recommendations.
...
# MAGIC %md
# MAGIC # 4. Usage
# MAGIC We demonstrate the usage of the **get_recommendations** function by generating recommendations for a user with ID 1.

# COMMAND ----------

# Example usage of the get_recommendations function for user with ID 1
user_id = "1"  # Ensure userId is a string
...
# MAGIC - **Surprise**: A Python scikit for building and analyzing recommender systems that deal with explicit rating data. It supports various algorithms including matrix factorization, k-NN, and baseline algorithms.
# MAGIC - **MLflow**: An open-source platform for managing the end-to-end machine learning lifecycle. It provides tools for tracking experiments, packaging code into reproducible runs, and sharing and deploying models.
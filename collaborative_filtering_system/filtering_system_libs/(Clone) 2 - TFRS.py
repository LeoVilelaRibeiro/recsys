...
with mlflow.start_run() as run:
  # Logar parâmetros
  mlflow.log_param("embedding_dimension", 32)
  mlflow.log_param("optimizer", "Adagrad")
  mlflow.log_param("learning_rate", 0.1)
  mlflow.log_param("epochs", 5)
  
  # Logar métricas
  for metric_name, metric_value in test_loss.items():
...
# Dados para predição (um único exemplo)
pred_data = {
  "userId": tf.constant(["user_1"]),
  "movieId": tf.constant(["movie_1"]),
  "category": tf.constant(["category_0"])
}

# Converter o dicionário para um TensorFlow Dataset
pred_ds = tf.data.Dataset.from_tensor_slices(pred_data).batch(1)
...
print(f"Predicted rating for user {pred_data['userId'].numpy()[0]} with category {pred_data['category'].numpy()[0]} and movie {pred_data['movieId'].numpy()[0]}: {predictions[0][0]}")
